...
## 🌐 5. Internacionalização (i18n) e Arquitetura de Idiomas

O sistema foi preparado de raiz para suportar multilinguismo (Português do Brasil - **PT-BR** e Inglês - **EN**) com tempo de resposta instantâneo (refresh-free) e reatividade automática da UI através de:

1. **State Store Interceptador:** O Zustand monitora o idioma ativo (`locale`) e sua persistência em `localStorage`.
2. **Abstração Transparente nos Hooks:** Hooks customizados como [`usePoliticalData.ts`](file:///c:/Users/joaop/antigravity%20web%20projects/politcmap/lib/hooks/usePoliticalData.ts) e [`useCountryDetail.ts`](file:///c:/Users/joaop/antigravity%20web%20projects/politcmap/lib/hooks/useCountryDetail.ts) resolvem internamente o idioma ativo. Eles filtram e selecionam as strings corretas de `description` e `key_events` baseados no locale da store, servindo tipos limpos de `string` para os componentes e poupando complexidade na renderização da interface.
3. **Mecanismo Antifalha de Hidratação:** O [`I18nProvider.tsx`](file:///c:/Users/joaop/antigravity%20web%20projects/politcmap/components/providers/I18nProvider.tsx) garante que o primeiro ciclo de renderização no servidor (SSR) e hidratação no cliente coincidam no idioma padrão (`pt`), chaveando para o idioma do usuário apenas pós-montagem no navegador (`useEffect`).

---

## 📊 6. Integração e Pipeline de Dados V-Dem

Para sair de dados fictícios e adotar registros históricos reais de alta fidelidade científica, criamos um pipeline automatizado de ingestão e normalização baseado nos dados oficiais do **V-Dem (Varieties of Democracy) v16**:

```mermaid
flowchart LR
    VDEM[V-Dem ZIP CSV] -->|convert-vdem.ts| Parse[CSV Line Reader]
    Parse -->|Filtra ISO-3 para ISO-2| Map[Mapeamento Geográfico]
    Map -->|Média de Anos por Período| Avg[Cálculo de Médias]
    Avg -->|Fórmula de Calibração| Calib[Cálculo do Espectro e Regimes]
    Calib -->|Google Translate API Cache| Trans[Tradução Automática PT/EN]
    Trans -->|Sobrescreve| Dataset[staticDataset.ts]
    Dataset -->|validate-dataset.ts| Valida[Sucesso: 0 Gaps / 100% Coerência]
```

### Otimizações do Parser de Ingestão:
* **Remoção de Colunas de Backtracking:** O CSV do V-Dem contém colunas textuais complexas com aspas e vírgulas internas. O parser remove cirurgicamente a primeira coluna (`country_name`) antes de realizar a quebra de string da linha, diminuindo o processamento para menos de `1 segundo`.
* **Calibração de Espectro Político:** A escala do V-Dem (`v2x_polyarchy` e `v2x_libdem`) varia de 0 a 1. Nossa fórmula de calibração converte esses índices para a escala contínua de `-10` a `+10` da aplicação, cruzando com a orientação original dos líderes para calibrar o sinal cromático sem perder a identidade histórica esquerda-direita da governança do país.

---

## 🗺️ 7. Guia de Migração: Banco de Dados e API Dinâmica
